...
export default function HealthScoreCard({ healthScore }: HealthScoreCardProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 flex flex-col gap-5">
      {/* Header row */}
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 mb-0.5">Business Health Score</p>
          <p className="text-xs text-gray-500 dark:text-slate-400">Composite score across 5 weighted dimensions</p>
        </div>
        <div className="text-right">
          <p className="text-4xl font-black text-gray-900 dark:text-slate-100 leading-none">{healthScore.overall}</p>
          ...
          <div key={dim.label}>
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-gray-700 dark:text-slate-300">{dim.label}</span>
                <span className="text-[10px] text-gray-400 dark:text-slate-500">({Math.round(dim.weight * 100)}%)</span>
              </div>
              <span className="text-xs font-bold text-gray-800 dark:text-slate-200">{dim.score}</span>
            </div>
      ...
      <p className="text-[10px] text-gray-400 dark:text-slate-500 leading-relaxed border-t border-gray-100 dark:border-slate-700 pt-3">
